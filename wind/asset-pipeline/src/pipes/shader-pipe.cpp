...
void ShaderPipe::compile(const fs::path& _source, const fs::path& _destination) {
    ...
    // vtx
    {
        auto result = compress(reinterpret_cast<Bytef*>(vtxZipped), &vtxZippedSize,
                               reinterpret_cast<const Bytef*>(vtxShader), vtxSize);
        if (result != Z_OK) {
    ...
    // fgt
    {
        auto result = compress(reinterpret_cast<Bytef*>(fgtZipped), &fgtZippedSize,
                               reinterpret_cast<const Bytef*>(fgtShader), fgtSize);
        if (result != Z_OK) {
            spdlog::error("[shader-pipe] Cannot compress data: {}", (int)result);
            return;
        }
